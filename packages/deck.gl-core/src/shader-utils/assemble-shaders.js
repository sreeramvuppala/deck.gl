...
// Load shader chunks
// import SHADER_MODULES from '../../dist/shaderlib/shader-chunks';
import SHADER_MODULES from '../shaderlib';
...
function assembleShader(gl, {
  ...
  for (const chunkName of Object.keys(SHADER_MODULES)) {
    if (opts[chunkName]) {
      source += `${SHADER_MODULES[chunkName].source}\n`;
    }
  }
  source += vs;
  return source;
